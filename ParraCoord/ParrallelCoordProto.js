function parraCoord(dataUp) {

    //On efface pour mieux redessiner =)
    commeNeuf();

    var traits = d3.keys(dataUp[0][0]);
    //console.log(traits2);
...
//d3.csv("Data/data4.csv", function(flowers) {

  var flowers = dataUp[0];
...
var coul = d3.scale.log().domain([d3.min(tabTailleSansZero),d3.max(tabTaille)]).range(["red","blue"]);
  ...
  foreground = svg.append("g")
    ...
    .enter().append("svg:path")
      ...
      .style("stroke",function(d){ 
        if (d.taille == 0) { return "#ccff99"; }
        else { return coul(d.taille); }
      });
...
d3.select("div.legendG").append("div")
                        ...
                        .text("Les traits verts correspondes aux prototypes de taille = 0.");

d3.select("div.legendG").append("div")
                        .style("height","20%")
                        .style("width","99%")
                        .style("font-size","20px")
                        .text("Les autres suivent une echelle logarithmique croissante du rouge vers le bleu.");
